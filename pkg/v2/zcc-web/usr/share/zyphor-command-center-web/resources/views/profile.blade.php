...
<link rel="stylesheet" href="{{ url('assets/bootstrap/all.min.css') }}">
        ...
        <div class="panel">
          ...
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <input type="text" class="form-control" id="fName" value="{{ $userInfo['name'] }}" maxlength="40">
            </div>
            <div class="col-md-6">
              <label class="field-label">Username</label>
              <input type="text" class="form-control mono" id="fHandle" value="{{ $userName }}" readonly>
              <div class="hint">Derived from the system account, not editable.</div>
            </div>
            <div class="col-md-6">
              <label class="field-label">Email</label>
              <input type="email" class="form-control" id="fEmail" value="{{ $userInfo['email'] }}">
            </div>
            <div class="col-12">
              <label class="field-label">Bio</label>
              <textarea class="form-control" id="fBio" placeholder="Say something about what you build...">{{ $userInfo['bio'] }}</textarea>
            </div>
          </div>
        </div>
...
$(function () {
  ...
  $('#btnRemoveAvatar').on('click', function () {
    $.ajax({
      ...
      method: 'DELETE',
      success: function (res) {
        $('#avatarPreview').html('{{ strtoupper(substr($userName, 0, 2)) }}');
        toast(res.message);
      },
